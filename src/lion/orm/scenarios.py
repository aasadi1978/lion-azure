...
class Scenarios(LION_SQLALCHEMY_DB.Model):
    ...
    scn_name = LION_SQLALCHEMY_DB.Column(LION_SQLALCHEMY_DB.String(255), nullable=False, default='ScnPlaceHolder')
    ...
    timestamp = LION_SQLALCHEMY_DB.Column(LION_SQLALCHEMY_DB.DateTime, default=utcnow())
    ...
    def check_password(cls, password):
        ...
        if existing_obj:
            if str(existing_obj.user_id) == str(SESSION_MANAGER.get('user_id')):
                return True
    ...
    def fetch_scn_name(cls, scn_id=None):
        try:
            ...
            log_exception(popup=False, remarks=f'failed to get scn_name: {str(e)}')
            return None
        except Exception:
