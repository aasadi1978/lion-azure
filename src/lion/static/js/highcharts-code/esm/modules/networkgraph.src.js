...
const external_highcharts_src_js_default_namespaceObject =
  __WEBPACK_EXTERNAL_MODULE__highcharts_src_js_8202131d__['default'];
var external_highcharts_src_js_default_default =
  /*#__PURE__*/ __webpack_require__.n(
    external_highcharts_src_js_default_namespaceObject
  ); // external ["../highcharts.src.js","default","SVGElement"]
const external_highcharts_src_js_default_SVGElement_namespaceObject =
  __WEBPACK_EXTERNAL_MODULE__highcharts_src_js_8202131d__['default'].SVGElement;
var external_highcharts_src_js_default_SVGElement_default =
  /*#__PURE__*/ __webpack_require__.n(
    ...
    mousedownUnbinder = addEvent(chart.container, 'mousedown', (event) => {
      ...
      if (
        ...
        mousemoveUnbinder = addEvent(
          chart.container,
          'mousemove',
          (e) => point && point.series && point.series.onMouseMove(point, e)
        );
...
/* harmony default export */ const Series_DragNodesComposition =
  DragNodesComposition; // ./code/es-modules/Series/GraphLayoutComposition.js
...
const { composed: GraphLayoutComposition_composed } =
  external_highcharts_src_js_default_default();

const {
  addEvent: GraphLayoutComposition_addEvent,
  pushUnique: GraphLayoutComposition_pushUnique,
} = external_highcharts_src_js_default_default();
/* *
 *
 *  Constants
...
/* harmony default export */ const Series_GraphLayoutComposition =
  GraphLayoutComposition; // external ["../highcharts.src.js","default","SeriesRegistry"]

const external_highcharts_src_js_default_SeriesRegistry_namespaceObject =
  __WEBPACK_EXTERNAL_MODULE__highcharts_src_js_8202131d__['default']
    .SeriesRegistry;
var external_highcharts_src_js_default_SeriesRegistry_default =
  /*#__PURE__*/ __webpack_require__.n(
...
const {
  series: {
    ...
    prototype: {
      pointClass: { prototype: pointProto },
    },
  },
} = external_highcharts_src_js_default_SeriesRegistry_default();

const { defined, extend, find, merge, pick } =
  external_highcharts_src_js_default_default();
/* *
 *
 *  Composition
 ...
 *
 * */
/* harmony default export */ const Series_NodesComposition = NodesComposition; // ./code/es-modules/Series/Networkgraph/NetworkgraphPoint.js
...
const {
  series: {
    prototype: NetworkgraphPoint_seriesProto,
    prototype: { pointClass: Point },
  },
} = external_highcharts_src_js_default_SeriesRegistry_default();

const {
  addEvent: NetworkgraphPoint_addEvent,
  css,
  defined: NetworkgraphPoint_defined,
  extend: NetworkgraphPoint_extend,
  pick: NetworkgraphPoint_pick,
} = external_highcharts_src_js_default_default();
/* *
 *
 *  Class
...
class NetworkgraphPoint extends Point {
   ...
   */
  getDegree() {
    const deg = this.isNode ? this.linksFrom.length + this.linksTo.length : 0;
    return deg === 0 ? 1 : deg;
  }
  /**
 ...
 *
 * */
/* harmony default export */ const Networkgraph_NetworkgraphPoint =
  NetworkgraphPoint; // ./code/es-modules/Series/Networkgraph/NetworkgraphSeriesDefaults.js
 ...
 *
 * */
/* harmony default export */ const Networkgraph_NetworkgraphSeriesDefaults =
 ...
 */
(''); // Adds doclets above to transpiled file // ./code/es-modules/Series/Networkgraph/EulerIntegration.js
...
function barycenter() {
  ...
  this.nodes.forEach(function (node) {
    if (!node.fixedPosition) {
      ...
      node.dispX +=
        ((xFactor - node.plotX) * gravitationalConstant * phi) / node.degree;
      node.dispY +=
        ((yFactor - node.plotY) * gravitationalConstant * phi) / node.degree;
    }
  });
}
/**
...
/* harmony default export */ const Networkgraph_EulerIntegration =
  EulerIntegration; // ./code/es-modules/Series/Networkgraph/QuadTreeNode.js
 ...
 *
 * */
/* harmony default export */ const Networkgraph_QuadTreeNode = QuadTreeNode; // ./code/es-modules/Series/Networkgraph/QuadTree.js
 ...
 *
 * */
/* harmony default export */ const Networkgraph_QuadTree = QuadTree; // ./code/es-modules/Series/Networkgraph/VerletIntegration.js
...
function VerletIntegration_integrate(layout, node) {
  const friction = -layout.options.friction,
...
/* harmony default export */ const Networkgraph_VerletIntegration =
  VerletIntegration; // ./code/es-modules/Series/Networkgraph/ReingoldFruchtermanLayout.js
...
const {
  clamp,
  defined: ReingoldFruchtermanLayout_defined,
  isFunction,
  fireEvent,
  pick: ReingoldFruchtermanLayout_pick,
} = external_highcharts_src_js_default_default();
...
class ReingoldFruchtermanLayout {
  ...
  repulsiveForces() {
    if (this.approximation === 'barnes-hut') {
      for (const node of this.nodes) {
        this.quadTree.visitNodeRecursive(null, (quadNode) =>
          this.barnesHutApproximation(node, quadNode)
        );
      }
    } else {
  ...
  isStable() {
    return (
      Math.abs(this.systemTemperature - this.prevSystemTemperature) < 0.00001 ||
      this.temperature <= 0
    );
  }
 ...
 *
 * */
/* harmony default export */ const Networkgraph_ReingoldFruchtermanLayout =
  ReingoldFruchtermanLayout; // ./code/es-modules/Series/SimulationSeriesUtilities.js
...
const { merge: SimulationSeriesUtilities_merge, syncTimeout } =
  external_highcharts_src_js_default_default();
...
/* harmony default export */ const SimulationSeriesUtilities =
  DataLabelsDeferUtils; // ./code/es-modules/Extensions/TextPath.js
...
const {
  addEvent: TextPath_addEvent,
  merge: TextPath_merge,
  uniqueKey,
  defined: TextPath_defined,
  extend: TextPath_extend,
} = external_highcharts_src_js_default_default();
/**
...
function setPolygon(event) {
  ...
  if (tp) {
    ...
    const appendTopAndBottom = (charIndex, positionOfChar) => {
      ...
      return [
        [x - descender * cosRot, y - descender * sinRot],
        [x + b * cosRot, y + b * sinRot],
      ];
    };
    for (let i = 0, lineIndex = 0; lineIndex < numOfLines; lineIndex++) {
      ...
      for (let lineCharIndex = 0; lineCharIndex < lineLen; lineCharIndex += 5) {
        try {
          const srcCharIndex = i + lineCharIndex + lineIndex,
      ...
      try {
        const srcCharIndex = i + lineIndex,
          ...
          [lower, upper] = appendTopAndBottom(srcCharIndex, charPos);
        polygon.unshift(upper);
...
function drawTextPath(event) {
  const labelOptions = event.labelOptions,
    point = event.point,
    textPathOptions =
      labelOptions[point.formatPrefix + 'TextPath'] || labelOptions.textPath;
...
/* harmony default export */ const Extensions_TextPath = TextPath; // ./code/es-modules/Series/Networkgraph/NetworkgraphSeries.js
...
const {
  ...
  initDataLabelsDefer: NetworkgraphSeries_initDataLabelsDefer,
} = SimulationSeriesUtilities;

const {
  addEvent: NetworkgraphSeries_addEvent,
  defined: NetworkgraphSeries_defined,
  extend: NetworkgraphSeries_extend,
  merge: NetworkgraphSeries_merge,
  pick: NetworkgraphSeries_pick,
} = external_highcharts_src_js_default_default();
...
class NetworkgraphSeries extends Series {
    ...
    if (this.options.nodes) {
      this.options.nodes.forEach(function (nodeOptions) {
        if (!this.nodeLookup[nodeOptions.id]) {
          this.nodeLookup[nodeOptions.id] = this.createNode(nodeOptions.id);
        }
      }, this);
    }
    ...
    if (point && !point.isNode) {
      ...
      if (stateOptions) {
        attribs = {
          ...
          'stroke-width': stateOptions.linkColor || attribs['stroke-width'],
        };
      }
    }
    return attribs;
  ...
  render() {
    ...
    if (series.chart.hasRendered && !series.options.dataLabels.allowOverlap) {
...
external_highcharts_src_js_default_SeriesRegistry_default().registerSeriesType(
  'networkgraph',
  NetworkgraphSeries
);
/* *
 ...
 *
 * */
/* harmony default export */ const Networkgraph_NetworkgraphSeries =
  NetworkgraphSeries;
/* *
 *
 *  API Declarations
 ...
 */
(''); // Detach doclets above // ./code/es-modules/masters/modules/networkgraph.src.js

const G = external_highcharts_src_js_default_default();
Networkgraph_NetworkgraphSeries.compose(G.Chart);
/* harmony default export */ const networkgraph_src =
  external_highcharts_src_js_default_default();
