...
class RuntimesMileages(LION_SQLALCHEMY_DB.Model):
    ...
    __tablename__ = 'DistanceTime'
    
    runtimes_cache = TTLCache(maxsize=1000, ttl=3600 * 8)

    orig = LION_SQLALCHEMY_DB.Column('Origin', LION_SQLALCHEMY_DB.String(255), primary_key=True, nullable=False)
    dest = LION_SQLALCHEMY_DB.Column('Destination',
                     LION_SQLALCHEMY_DB.String(255), primary_key=True, nullable=False)
    ...
    remarks = LION_SQLALCHEMY_DB.Column(
        'Remarks', LION_SQLALCHEMY_DB.String(255), nullable=True)
